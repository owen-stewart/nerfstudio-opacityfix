...
# OPTIMIZED TRAINING PARAMS THAT WE ARE USING 

# means_crop — the XYZ positions of all Gaussians in world space, shape [N, 3]
# quats_crop — the rotation quaternions of each Gaussian, shape [N, 4]
# scales_crop — the log-scale values of each Gaussian's three axes, shape [N, 3]
# opacities_crop — the raw logit opacity of each Gaussian, shape [N, 1]
# colors_crop — the spherical harmonic colour coefficients, shape [N, D, 3]
# optimized_camera_to_world — the 4x4 matrix describing where the camera is in world space


        # STEP 1: VIEW DIRECTION

        # Compute normalised view direction from camera to each Gaussian.
        # Camera position extracted from camera-to-world matrix (top-right 3x1 block).
        # Detached to prevent gradient flow — forward-pass correction only.

        camera_pos = optimized_camera_to_world[0, :3, 3]
        view_dirs = means_crop.detach() - camera_pos
        view_dirs = view_dirs / (view_dirs.norm(dim=-1, keepdim=True) + 1e-8)

        # STEP 2: RADIAL NORMAL (correct for sphere geometry)
        # Estimate surface normals using radial directions from scene center.
        # Center estimated as mean of all Gaussian positions.
        # Exact for spheres — incorrect for non-spherical objects (cubes, prisms).

        scene_center = means_crop.detach().mean(dim=0)
        radial = means_crop.detach() - scene_center
        normals = radial / (radial.norm(dim=-1, keepdim=True) + 1e-8)

        # STEP 3: DISTANCE + FLATNESS GATE
        # Identify glass Gaussians using two heuristic filters:
        # (1) Distance gate — Gaussians within 70-130% of median radius (sphere shell).
        # (2) Flatness gate — removes near-spherical fog blobs (s_min/s_max < 0.3).
        # Both conditions must be satisfied. (not geometrically perfect).

        dists = radial.norm(dim=-1)
        median_dist = dists.median()
        inner = median_dist * 0.7
        outer = median_dist * 1.3
        scale_exp = torch.exp(scales_crop.detach())
        scale_min = scale_exp.min(dim=-1).values
        scale_max = scale_exp.max(dim=-1).values
        flatness_ratio = scale_min / (scale_max + 1e-8)
        glass_mask = (((dists > inner) & (dists < outer)) & (flatness_ratio < 0.3)).float().unsqueeze(-1)

        # STEP 4: SCHLICK FRESNEL
        ...
        cos_theta = (view_dirs * normals).sum(dim=-1, keepdim=True).abs()
        ...
        # T = 1 - F(theta). Scale factor 0.30 tuned via ablation study.
        # Non-glass Gaussians unchanged. Result converted back to logit space.
        # Affects rendering only — training opacities remain inflated.

        OPACITY_SCALE = 0.30
        transmittance = 1.0 - fresnel
        raw_opacity = torch.sigmoid(opacities_crop)
        suppressed = raw_opacity * transmittance * OPACITY_SCALE
        final_opacity = raw_opacity * (1.0 - glass_mask) + suppressed * glass_mask
        final_opacity = torch.clamp(final_opacity, 1e-6, 1.0 - 1e-6)
        opacities_crop = torch.log(final_opacity / (1.0 - final_opacity))

        # STEP 6: SPECULAR TINT
        ...
        # Cosmetic only — approximates highlights without true reflection modelling.

        specular_tint = fresnel * glass_mask * 0.15
        if sh_degree_to_use is None:
            colors_crop = torch.clamp(colors_crop + specular_tint, 0.0, 1.0)
        else:
            colors_crop = colors_crop.clone()
            colors_crop[:, 0, :] = colors_crop[:, 0, :] + (specular_tint / 0.28209)
